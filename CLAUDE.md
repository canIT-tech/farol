...
- Providers reais: **Travelpayouts** (voo via Aviasales — data + deep-link com afiliado) + **Google Places** (POI/restaurantes). Amadeus Self-Service foi descontinuado (só Enterprise). **Hotel segue sem provider**: o Hotellook foi encerrado em 20/10/2025 (marca, programa de afiliado e API) — sem credencial da Amadeus a seção de hotel degrada e o resto do roteiro segue.
...
- **Hotel sem provider — e sem caminho pelo Travelpayouts.** O **Hotellook foi encerrado como marca em 20/10/2025**, junto com o programa de afiliado e a API (`engine.hotellook.com` responde 404 em tudo, inclusive na raiz — é CloudFront sem origem, não falta de permissão). **Não adianta pedir liberação.** O Travelpayouts hoje intermedia hotel só como rede de afiliados (Booking.com, Agoda, Trip.com), sem API de dados própria. Opções: (a) afiliado de hotel via Travelpayouts, com deep-link e sem preço na nossa UI; (b) API de dados de outro fornecedor (Amadeus Enterprise, Hotelbeds, LiteAPI); (c) manter a seção degradada até o v1. Hoje `HOTEL_PROVIDER` só monta a Amadeus se `AMADEUS_CLIENT_ID/SECRET` existirem; sem elas a seção devolve `error: "unavailable"` e o roteiro segue.
